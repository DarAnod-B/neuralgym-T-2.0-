...
class ModelRestorer(OnceCallback):
    ...
    def __init__(self, saver, dump_prefix=None, ckpt_file=None, optimistic=False):
        ...
        if ckpt_file is None:
            ...
            # get ckpt file.
            self._ckpt_file = tf.train.latest_checkpoint(os.path.dirname(dump_prefix))
    ...
    def run(self, sess):
        def optimistic_restore(sess, ckpt_file):
            ...
            with tf.name_scope("", reuse=True):
                for var_name, saved_var_name in var_names:
                    curr_var = name2var[saved_var_name]
                    var_shape = curr_var.get_shape().as_list()
                    if var_shape == saved_shapes[saved_var_name]:
                        restore_vars.append(curr_var)
                        print("- restoring variable: {}".format(curr_var.name))
            saver = tf.train.Saver(restore_vars)
            saver.restore(sess, ckpt_file)

        if not self._no_ckpt_file:
            callback_log("Trigger ModelRestorer: Load model from %s." % self._ckpt_file)
            if self._optimistic:
