...
class ModelSync(PeriodicCallback):
    ...
    def __init__(self, pstep, from_namescope, to_namescope, step_start=False):
        ...
        vars_from = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, from_namescope)
        vars_to = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, to_namescope)
        # ops to sync
        self._ops = []
        callback_log(
            "Add callback: sync model from namescope: {} to namescope: {}".format(
                from_namescope, to_namescope
            )
        )
        with tf.name_scope(tf.get_variable_scope(), reuse=True):
            for var in vars_to:
                name = var.name
                if self.from_namescope == "":
                    # root
                    name = name.replace(self.to_namescope + "/", self.from_namescope)
                else:
                    name = name.replace(
                ...
                print(
                    "Add op for syncing from var: {} to var: {}".format(
                        from_var.name, var.name
                    )
                )
